...
import { createStorageBackup, restoreStorageBackup, verifyStorageBackup, pruneStorageBackups } from "../packages/storage/src/backups";
const {values}=parseArgs({args:process.argv.slice(2),options:{help:{type:"boolean"},create:{type:"boolean"},restore:{type:"boolean"},verify:{type:"boolean"},prune:{type:"boolean"},repository:{type:"string"},snapshot:{type:"string"}},strict:true});
if (values.help) {
  console.log("Create: --create --repository DIRECTORY\nVerify: --verify --repository DIRECTORY [--snapshot ID]\nPrune: --prune --repository DIRECTORY\nRestore: --restore --repository DIRECTORY [--snapshot ID]\nUses HV_PG_ADMIN_URL, HV_DATABASE_TLS_DIR, HV_S3_*, optional HV_PG_BIN/HV_PG_LIB. Restores require an empty offline database and separate private bucket. Only trusted operator backups may be restored.");process.exit(0);
}
if (!values.repository || [values.create,values.restore,values.verify,values.prune].filter(Boolean).length!==1) throw new Error("choose create, restore or verify and a repository; use --help");
if (values.prune) {
  console.log(JSON.stringify({pruned:true,...await pruneStorageBackups(values.repository)}));
